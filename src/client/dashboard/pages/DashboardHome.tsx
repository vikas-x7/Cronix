  ...
  return (
    <div className="w-full h-screen overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-6 py-6">
        <StatCard title="Total Jobs" value={stats?.jobs.total ?? 0} subtitle={`${stats?.jobs.active ?? 0} active · ${stats?.jobs.paused ?? 0} paused`} icon={<FiClock size={18} />} />
        <StatCard title="Active Jobs" value={stats?.jobs.active ?? 0} icon={<FiActivity size={18} />} trend="up" trendValue="Running" />
      ...
      <div className="px-6 py-4">
        ...
        {!stats?.recentExecutions?.length ? (
          <div className="border border-dashed border-[#D9D9D9] h-[40%] py-40 flex flex-col items-center justify-center">
            ...
            <p className="text-[11px] text-black mt-1">Create a cron job and trigger it to see results here</p>
          </div>
        ) : (
