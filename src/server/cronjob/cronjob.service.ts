...
export class CronJobService {
  ...
  async create(userId: string, data: CreateCronJobInput) {
    ...
    try {
      nextExecutionAt = getNextCronDate(data.cronExpression, data.timezone);
    } catch {
      // Default behavior if expression is invalid
    }
  ...
  async update(id: string, userId: string, data: UpdateCronJobInput) {
    ...
    if (data.headers !== undefined)
      updateData.headers = data.headers ?? undefined;
    ...
    // Recalculate next execution if cron or timezone changes, or status changed to active
    if (
      ...
      try {
        const expr = data.cronExpression ?? existing.cronExpression;
        const tz = data.timezone ?? existing.timezone;
        updateData.nextExecutionAt = getNextCronDate(expr, tz);
      } catch {
        // Ignore calculate error
      }
    } else if (updateData.status === "paused") {
      updateData.nextExecutionAt = null; // Don't execute if paused
    }
  ...
  async toggleStatus(id: string, userId: string) {
    ...
    let nextExecutionAt: Date | null | undefined = undefined;

    if (newStatus === "active") {
      // Active karte waqt next execution calculate karo
      try {
        nextExecutionAt = getNextCronDate(
      ...
      // Paused karte waqt null karo
      nextExecutionAt = null;
    }

    return cronJobRepository.update(id, {
      status: newStatus,
      nextExecutionAt,
    });
  }
  ...
  async triggerManually(id: string, userId: string) {
    ...
    try {
      ...
      if (job.body && !["GET", "HEAD"].includes(job.method)) {
        fetchOptions.body =
          typeof job.body === "string" ? job.body : JSON.stringify(job.body);
      }
