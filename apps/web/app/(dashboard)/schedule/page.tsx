...
type Tab = 'Headers' | 'Body' | 'Schedule';
      ...
      <div className="bg-[#1F1F1F] rounded-[5px] h-[92vh] flex flex-col">
        ...
        <div className="px-4 py-3 flex gap-2">
          ...
          <button
            ...
            className="bg-white text-black px-5 rounded-[3px] text-[13px] font-medium transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-neutral-200 cursor-pointer"
          >
        ...
        <div className="flex px-4 border-b border-neutral-800 gap-6 text-[13px] font-medium">
          {(['Schedule', 'Headers', 'Body'] as Tab[]).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`py-2.5 relative transition cursor-pointer ${activeTab === tab ? 'text-white' : 'text-neutral-500 hover:text-white'}`}
            >
              {tab}
              {activeTab === tab && (
              ...
              <div>
                ...
                <div className="border border-[#393939] rounded-[3px] p-4">
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={() => updateField('failureEmail', !formData.failureEmail)}
                      className={`h-5 w-9 rounded-full transition cursor-pointer shrink-0 relative ${formData.failureEmail ? 'bg-white' : 'bg-neutral-700'}`}
                    >
                      <span className={`block h-4 w-4 rounded-full transition absolute top-0.5 ${formData.failureEmail ? 'left-[18px] bg-black' : 'left-0.5 bg-neutral-400'}`} />
                    </button>
                  ...
                  {formData.failureEmail && (
                    <div className="ml-12 mt-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[12px] text-neutral-400">Notify after</span>
                        <input
                          type="number"
                          min={1}
                          value={formData.failureThreshold}
                          onChange={(e) => updateField('failureThreshold', Number(e.target.value))}
                          className="w-16 bg-neutral-800 border border-[#393939] rounded-[3px] px-2 py-1 text-[13px] text-white outline-none focus:border-neutral-500"
                        />
                        <span className="text-[12px] text-neutral-400">failure(s)</span>
                      </div>
                    </div>
                  )}
                  ...
                  <NotifyToggle
                    ...
                    onChange={() => updateField('notifyOnRecovery', !formData.notifyOnRecovery)}
                  ...
                  <NotifyToggle
                    ...
                    onChange={() => updateField('disableAfterFailures', !formData.disableAfterFailures)}
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={() => updateField('notifyTlsExpiry', !formData.notifyTlsExpiry)}
                    ...
                    >
                      <span className={`block h-4 w-4 rounded-full transition absolute top-0.5 ${formData.notifyTlsExpiry ? 'left-[18px] bg-black' : 'left-0.5 bg-neutral-400'}`} />
                    </button>
                    <p className="text-[13px] text-white">TLS certificate expiring</p>
                  </div>
                  {formData.notifyTlsExpiry && (
                    <div className="ml-12 mt-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[12px] text-neutral-400">Notify before</span>
                        <input
                          type="number"
                          min={1}
                          value={formData.tlsExpiryDays}
                          onChange={(e) => updateField('tlsExpiryDays', Number(e.target.value))}
                          className="w-16 bg-neutral-800 border border-[#393939] rounded-[3px] px-2 py-1 text-[13px] text-white outline-none focus:border-neutral-500"
                        />
...
function NotifyToggle({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        <span className={`block h-4 w-4 rounded-full transition absolute top-0.5 ${checked ? 'left-[18px] bg-black' : 'left-0.5 bg-neutral-400'}`} />
      </button>
