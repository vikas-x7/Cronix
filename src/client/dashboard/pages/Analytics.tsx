    ...
    <div className="w-full h-screen overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-6 py-6">
        ...
        <StatCard
          ...
          trend={(stats?.executions.failedExecutions ?? 0) > 0 ? 'down' : 'neutral'}
          trendValue={(stats?.executions.failedExecutions ?? 0) > 0 ? 'Needs attention' : 'All good'}
        />
        <StatCard
          title="Success Rate"
          ...
          trendValue={(stats?.executions.successRate ?? 0) >= 90 ? 'Healthy' : (stats?.executions.successRate ?? 0) >= 50 ? 'Moderate' : 'Critical'}
        />
      </div>
                ...
                {jobsWithExecutions
                  ...
                  .map((job) => (
                    <div key={job.id} className="flex items-center justify-between">
                      <span className="text-[12px] text-neutral-600 truncate max-w-[140px]">{job.title}</span>
                      <span className="text-[12px] font-mono text-neutral-400">{job._count.executions}</span>
                    </div>
                  ))}
              </div>
            )}
